...
## 10. Change log
- **v1.5 — 24 Jul 2026.** **`attribution/v2` implemented** (roadmap V3+V4, ADR 0013): creation-time AI-contribution attribution as running, property-tested code. AI musicians (Kiekus) are first-class registry citizens with training-licensor shares; creator share is *earned* from session note telemetry (floor/ceiling interpolation, monotone in human contribution); ingredient pool weighted by presence in the final mix; AI portions flow through style lineage to licensor artists. 104 tests green. Migration 0004 (AI-musician tables + telemetry columns) committed; **not yet applied** — Supabase project auto-paused and restore blocked by the free plan's 2-active-project cap (T7 in open-questions; needs a decision: pause another project, upgrade, or wait). v1 stays the API default until the UE5/Kieku capture spec ships telemetry.
- **v1.4 — 24 Jul 2026.** Innovation track made first-class (directive: the engine must contain something genuinely novel, not only correct foundations). Researched Kieku AI online (real-time next-note prediction over MIDI, attention-based; marketplace of artist-licensed "AI musicians"; public promise of royalties to style licensors). Defined the novel core: **creation-time AI-contribution attribution** — ground-truth note-level provenance captured while the music is made (Burst is the instrument; no post-hoc detection), flowing through sample rights AND AI-musician style lineage to pay creator, sample owners, and training-data artists from one deterministic ledger. Roadmap + action items V1–V5 in repo `docs/innovation-roadmap.md`; tracked as I1 in `docs/open-questions.md`. `attribution/v2` is where the R&D lives; novelty claim pending prior-art verification (V1).
- **v1.3 — 23 Jul 2026.** Business decisions taken and encoded: **platform fee 60% of gross** (Roblox retains ~72%), creator-vs-samples attribution stays 50/50, so a creator nets 40¢ per €1 on fully original work and 20¢ when sampling registered content — with the sampled rights-holders paid from the remainder, and Burst's own sample library earning through the samples share on top of the fee (ADR 0009). **Rights engine gets a dedicated Supabase project**; game identity crosses via verified JWT so no service-role key ever sits in a game build (ADR 0010). **Phase 5 complete** — license enforcement (flags rather than blocks; any covering license clears an asset; unknown territory never clears a restricted license) and reporting (statements as pure period folds, payout rows/CSV in integer minor units) — ADR 0011. 67 tests green.
- **v1.2 — 23 Jul 2026.** Pushed to GitHub: **https://github.com/rafaelmocelin-burst/burst-rights-engine** (public; branch protection on `main` requiring the CI `test` check). Domain sides of Phases 1–4 complete, 49 tests green: Phase 1 rights registry (append-only versioned splits, effective-dated deterministic resolution — ADR 0005); Phase 2 backend ingest (canonical entries or conservative `FTimelineSnapshot` extraction, unresolved refs flagged, idempotent on event id — ADR 0006; UE5 thin-client half pending); Phase 3 attribution engine `attribution/v1` (creator share + equal-weight ingredient pool, exact-ppm, property-tested — ADR 0007); Phase 4 royalty computation `royalty/v1` (fee → pools → conservation-exact allocation, reversal symmetry — ADR 0008); plus an end-to-end test of the full chain (the EIC demo story). Still pending: Supabase project decision + deploy, versioned HTTP API layer, async workers, UE5 client subsystem, Phase 5 licensing/reporting, Phase 6 hardening/demo.
